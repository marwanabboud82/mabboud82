...
def ReadCCS (TrackerFilePath):

    # DM STD Tracker File
    CCSPath = (TrackerFilePath.loc['DMSTDPath'] + 'CCS\\0303d_36.txt')

    CCS_Data = pd.read_csv(CCSPath[0],sep='\,',skiprows=15, header=None) # read securities
    ...
    CCS_Data_Cols = pd.read_fwf(CCSPath[0],colspecs,nrows=8,skiprows=3) # read column names  
